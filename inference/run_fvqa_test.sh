#!/bin/bash
# 数据集路径（请替换为实际路径）
DATASET=/your/dataset/path/fvqa_test.json
# 输出路径（请替换为实际路径）
OUTPUT_PATH=/your/output/path/fvqa_test_qwen/qwen32b-newagent

# 测试缓存目录（请替换为实际路径）
export TEST_CACHE_DIR="/your/cache/path/FVQA_Cache/fvqa_test_cache"
...
# Convert main_ports array to comma-separated string
MAIN_PORTS_STR=$(IFS=,; echo "${main_ports[*]}")

python -u run_multi_react.py --dataset "$DATASET" --output "$OUTPUT_PATH" --max_workers $MAX_WORKERS --max_tokens $MAX_TOKENS --model $MODEL_PATH --temperature $TEMPERATURE --presence_penalty $PRESENCE_PENALTY --top_p $TOP_P --total_splits ${WORLD_SIZE:-1} --worker_split $((${RANK:-0} + 1)) --roll_out_count $ROLLOUT_COUNT --main_ports "$MAIN_PORTS_STR"
